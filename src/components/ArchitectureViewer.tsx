  ...
  return (
    <div className="p-10 max-w-5xl mx-auto space-y-12 pb-32">
      <div className="flex items-center gap-5 border-b border-border pb-8">
        <div className="w-12 h-12 bg-indigo-600/10 rounded-full flex items-center justify-center border border-indigo-500/20 shadow-inner text-indigo-500">
           ...
           <button 
             ...
             className={cn(
               ...
               activeCategory === item.id 
                 ...
                 : "bg-muted/30 border-border text-muted-foreground hover:border-indigo-500/30 hover:bg-muted/50"
             )}
           >
             <div className={cn(
               ...
               activeCategory === item.id 
                 ...
                 : "bg-background border-border group-hover:text-indigo-500"
             )}>
        ...
        <AnimatePresence mode="wait">
          {loading && (
            <motion.div 
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="absolute inset-0 bg-background/50 backdrop-blur-sm z-10 flex items-center justify-center rounded-3xl"
            >
          ...
          <motion.div 
            ...
            className="bg-card border border-border p-10 rounded-3xl prose dark:prose-invert max-w-none leading-relaxed font-serif text-foreground/80"
          >
            {data ? data.split('\n').map((line, i) => (
